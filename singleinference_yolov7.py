...
class SingleInference_YOLOV7:
    ...
    def __init__(self, img_size, path_yolov7_weights, path_img_i='None', device_i='cpu', conf_thres=0.25, iou_thres=0.5):
    ...
    def load_model(self):
        self.model = self.attempt_load(
            self.path_yolov7_weights, map_location=self.device)  # load FP32 model
    ...
    def inference(self):
        if type(self.im) != type(None):
            ...
            self.outputs = self.non_max_suppression(
                self.outputs, self.conf_thres, self.iou_thres, classes=None, agnostic=False)
            img_i = self.im0.copy()
            ...
            for i, det in enumerate(self.outputs):
                if len(det):
                    ...
                    for j, (*bboxes, score, cls_id) in enumerate(reversed(det)):
                        ...
                        self.predicted_bboxes_PascalVOC.append(
                            [name, x0, y0, x1, y1, score])  # PascalVOC annotations
                        color = self.colors[self.names.index(name)]
                        name += ' '+str(score)
                        cv2.rectangle(
                            image, self.box[:2], self.box[2:], color, 2)
                        cv2.putText(
                            image, name, (self.box[0], self.box[1] - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.3, color, 1)
                    self.image = image
                else:
...
if __name__ == '__main__':
    ...
    app = SingleInference_YOLOV7(img_size, path_yolov7_weights,
                                path_img_i, device_i='cpu', conf_thres=0.25, iou_thres=0.5)
    #LOAD & INFERENCE
    app.load_model()  # Load the yolov7 model
    # read in the jpg image from the full path, note not required if you want to load a cv2matrix instead directly
    app.read_img(path_img_i)
